...
const ADOPTION_STATUSES = ["available", "pending", "adopted"];
...
const Step1 = ({ form, errors, set }) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
      ...
      <Field label="Adoption Status" error={errors.adoptionStatus}>
        <select
          value={form.adoptionStatus}
          onChange={(e) => set("adoptionStatus", e.target.value)}
          className={`${base} ${errors.adoptionStatus ? err : ok}`}
        >
...
const Step3 = ({ form, errors, set }) => (
  <div className="space-y-6">
    ...
    <div className="bg-orange-50 border border-orange-100 rounded-2xl p-5 text-sm text-orange-700">
      ...
      <p>
        Add a city or neighborhood so potential adopters nearby can find your
        post easily.
      </p>
    </div>
  </div>
...
const EditPost = ({ postId }) => {
  ...
  const { updatePost, getPetData } = useUserActions();
  ...
  useEffect(() => {
    ...
    const loadPet = async () => {
      const data = await getPetData({ id: postId });
      if (!data) return;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await updatePost({ formData: fd, postId });
      setForm(INITIAL);
      setStep(1);
    } finally {
      setIsLoading(false);
